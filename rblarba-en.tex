...
HyperKvasir is a more specialistic dataset that contains gastrointestinal images with some disabilities\todo{\tiny raczej np. \textit{various findings}, \textit{complications}, czy \textit{abnormal growths}}.
...
\todo[inline]{\tiny jakoś zaznacz że to tylko finalne modele, np. ``Overall, after various experiments with hyperparameters, the final setup included:''}


\section{Image classification on CIFAR-10 and HyperKvasir}

For image classification we used two sparse transformers (Swin and \sout{Performer}\todo{\tiny T2T\_ViT [with Performer attention]}) described in Section~\ref{r:sparse_transformers}, pretrained on the ImageNet dataset. We used also the plain Visual Transformer (ViT) \cite{DBLP:conf/iclr/DosovitskiyB0WZ21}. In later sections we will focus on differences between ViT and sparse transformers both in terms of performance and interpretability.
...
\todo[inline]{\small HyperKvasir jest mniejszy i łatwiejszy, dlatego używa dużo mocniejszych augmentacji i z powodu tych augmentacji ma stosunkowo niższe wyniki classifierów.}
...
\end{center}
\end{table}
\todo{\tiny Dodać do tabelki rozmiar każdego modelu (liczba parametrów)}
...
\todo[inline]{\small Trochę niezrozumiałe, najpierw bym powiedział że wszystko resize'ujemy do 224, potem że maskujemy np. ``either one of 4x4 or one of 16x16 patches, consisting of 56x56 or 16x16 pixels respectively.'', potem thus we have either 16 or 196 players, potem zalety 16, a na koniec dopisać zalety 196 (np. more precise and interpretable attributions).}

At the beginning, we present the performance of surrogate models.
Below\todo{gdzie?} are images from a batch of masked CIFAR-10 images, with marked predicted classes (green if correct, red for misclassification). One can notice that all three models make mistakes practically only for the completely or almost completely masked images.
...
\todo[inline]{\small W liczbach nie widzę żeby swin jakoś wyraźnie odstawał, można powiedzieć krócej że ma najlepsze wyniki na maskowanych inputach. Warto zwrócić uwagę że surrogate działają lepiej na nie-maskowanych inputach, zatem maskowanie działa jak augmentacja.}
...
\todo[inline]{\small Np. \textit{In the tabulated results, the average accuracy is reported, when each patch is masked independently with a probability of 50\%.}}
...
\todo[inline]{\small To raczej dlatego, że augmentacje classifiera na tym datasecie są mocniejsze.}
...
\end{figure}
\todo{\tiny Fig~\ref{true_swin_swin_cifar}: using the swin explainer, for the swin surrogate with 16 players on the CIFAR10 dataset?}
...
\todo[inline]{``solve'' w sensie optymalizujemy dwie zmienne $w$, czy znajdujemy wiele x dla jakoś ustalonych w? Z noisem chodzi o to żeby model nie mógł się nauczyć rozpoznawać maskę, tzn. które piksele są imputowane?}
...
\todo[inline]{\small Podkreśliłbym po pierwszym zdaniu, że lepsze explanation to takie, które ma mniejsze accuracy, można to też powtórzyć w pierwszym caption (przynajmniej ``lower accuracy means better explanation'').}
...
\label{fig:Saliency_Explainer_CIFAR}
\caption{Accuracy of surrogate model after removing features computed by explainer or saliency method on the CIFAR10 dataset.}
\end{figure}
